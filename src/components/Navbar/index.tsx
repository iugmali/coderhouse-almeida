    ...
    <>
      ...
      <nav className="sticky top-0 z-50 bg-gradient-to-b from-gray-800 to-gray-900 drop-shadow-lg text-gray-50">
        <div className="flex px-8 py-6 justify-between items-start mx-auto max-w-3xl">
          ...
          <Link href={`/cart`}><CartWidget quantity={cartItems.length} total={total} /></Link>
          <div className={`flex flex-col items-end justify-normal`}>
            ...
            <ul className={`${menuOpen ? 'flex flex-col' : 'hidden'} md:flex md:flex-row gap-8`}>
              <li className={`${(category === 'artigos') ? 'text-blue-400' : ''} hover:opacity-50 active:text-blue-400`}><Link href={'/artigos'}>Artigos</Link></li>
              <li className={`${(category === 'livros') ? 'text-blue-400' : ''} hover:opacity-50 active:text-blue-400`}><Link href={'/livros'}>Livros</Link></li>
              <li className={`${(category === 'videos') ? 'text-blue-400' : ''} hover:opacity-50 active:text-blue-400`}><Link href={'/videos'}>Vídeos</Link></li>
            </ul>
          </div>
        </div>
      </nav>
